...
define(['jquery', 'myalert'], function($) {
	...
	return {
		...
		getOutFun: function() {
			...
			window.location.href = this.omsBaseUrl + urls.allUrls.outLogin;
		},
		/**
		 * 文件上传
		...
		//统计图表
		chartFun: function(wrapData, chartContainter) {
			var mychart = echarts.init(chartContainter);
			var options = {
				tooltip: {
					trigger: 'axis'
				},
				...
				legend: {
					...
					data: wrapData.lengendData
				},
				series: wrapData.seriesData
			};
		 ...
		 * {h}  对象  正文内容
		 * */
		leftHeightFun: function(lD, rD, h) {
			$(h).height() > $(window).height() ? (function() {
				$(lD).css("height", $(h).height() + "px");
				$(rD).css("height", $(h).height() + "px");
			})() : (function() {
				$(lD).css("height", $(window).height() + "px");
				$(rD).css("height", $(window).height() + "px");
			})();
