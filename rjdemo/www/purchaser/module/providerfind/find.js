...
define(['js/libs/jquery.validate.min', 'js/libs/additional.methods', 'js/urls'], function(validates, methods, urls) {
	var register = function() {
		$(".header").remove();
		var findFirstUrl = urls.allUrls.getSupplierEmailLink;
		var findThirdUrl = urls.allUrls.setSuppNewPass;
			...
			submitHandler: function() {
				...
				var findPass = (function() {
					...
					var callbackGetFun = function(datas) {
						if(datas.code == "0000") {
							window.location.href = "#propasswordsecond?key=" + encodeURIComponent($.trim($("#number").val()), "utf8");
					...
					var callbackSetFun = function(datas) {
						if(datas.code == "0000") {
							window.location.href = "#propasswordforth";
						} else if(datas.code == "1020") {
							dialog({
								title: '提示',
								modal: true,
					...
					return {
						findPassGetCode: function() {
							common.postData(common.providerBaseUrl + findFirstUrl, getCodeData, callbackGetFun, true);
						},
						setNewPassMethod: function() {
							common.postData(common.providerBaseUrl + findThirdUrl, setNewPassData, callbackSetFun, true);
						}
					};
				})();
				//区分手机号码和邮箱
				if($("#find-form").attr("class") == "get-email") {
					if(emailReg.test(phoneNumberOrEmail)){
						...
						window.location.href="#promobilefirst?phone="+encodeURIComponent(phoneNumberOrEmail);	
					}
