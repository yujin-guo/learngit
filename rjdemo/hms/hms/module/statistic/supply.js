define(['text!module/statistic/supply.html',"text!module/statistic/header.html","text!module/statistic/nav.html","text!module/statistic/loadSupply.html","css!module/statistic/style/statistic.css","js/libs/laydate.dev"], function(tpl,header,nav,loadHtml) {
	var controller = function() {
		...
		var url=testUrl+urls.allUrls.suppStatistic;//品牌统计管理地址
		var pageSize = 20;
		var pageNo = 1;
		var requestData = {
			pageNo:1,
		...
		var callback=function(r){
			if(r.code="0000"){
				...
				$(".secondary-title").eq(2).addClass("secondary-active").siblings().removeClass("secondary-active");
				$(".secondary-title").eq(2).bind("click",function(){window.location.reload()});
			...
			var pageCallback=function(index){
				...
				var callback=function(r){
					if(r.code=="0000"){
						...
						$("#suppList").html(_.template(loadHtml,r))
						window.scrollTo(0,0)
					}
				}
		...
		function pageUpdate(data){
			...
			var callback=function(r){
				if(r.code=="0000"){
					...
					var totalP=r.totalPages*pageSize;
					$("#suppList").html(_.template(loadHtml,r));//列表更新
